...
function splitChapters(text) {
    ...
    // 如果有Markdown标题，用标题来分章节（优先用 # 或 ## 作为章节分割点）
    ...
    if (chapterStarts.length > 0) {
        ...
        for (var c = 0; c < chapterStarts.length; c++) {
            ...
            var body2 = lines.slice(startLine2, endLine2).join('\n').trim();
            chapters2.push({ title: chapterStarts[c].title, content: body2 || '(empty)' });
        }
        return chapters2;
    }
